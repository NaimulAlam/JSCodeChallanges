...
function convertToLowercase(str) {
  var lowerCaseConvertion = str.toLowerCase();
  return console.log(lowerCaseConvertion);
}

convertToLowercase("NAIMUL ALAM");
...
const fullName = (f_name, l_name) => {
  return console.log(f_name + " " + l_name);
};

fullName("Naimul", "Alam");
...
const convertToSquare = (num) => {
  return console.log(num * num);
};
convertToSquare(9);

const convertToAnyPower = (number, power) => {
  var result = Math.pow(number, power);
  return console.log(result);
};

convertToAnyPower(9, 3);
